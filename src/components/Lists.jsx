...
function UserLists() {
  ...
  React.useEffect(() => {
    db.getUserLists("an8kvgC3fgVUbZT2lrK2IyenWy33")
    .then((lists) =>{
      setLists(lists);
    })
  }, []);
...
function ListItem({list}) {
  const {id, name,issued, description, image, users} = list;
  return (
      ...
      {/* <Link to={`/id`}> */}
        <div className="flex relative">
          ...
          <div className="px-8 py-10 relative z-10 w-full border-4 border-gray-800 bg-gray-900 opacity-0 hover:opacity-100">
            ...
            <p className="leading-relaxed">Issued  :{issued == '' ? "available" : 'lent to ' + issued } </p>
          </div>
        </div>
      {/* </Link> */}
    </div>
